...
7. **Automatic Archiving:** All finished tracks MUST be automatically archived (moved to `conductor/archive/` and removed from `conductor/tracks.md`) without prompting for choice.
...
6. **Final Verification:**
   - Run the Gemini `test-fix` command (`/test-fix`) to ensure all project-wide checks (formatting, linting, and tests) pass after your changes.
...
### Before Committing
```bash
# Run all pre-commit checks using Gemini test-fix command
# /test-fix
