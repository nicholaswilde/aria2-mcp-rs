...
async fn test_notification_flow_multi_instance() -> Result<()> {
    ...
    client1.add_uri(vec!["https://example.com/file1.zip".to_string()], None).await?;
    let n1 = tokio::time::timeout(Duration::from_secs(5), rx.recv()).await?.unwrap();
    assert_eq!(n1.method, Aria2Event::DownloadStart);
    
    // Trigger event on box2
    client2.add_uri(vec!["https://example.com/file2.zip".to_string()], None).await?;
    let n2 = tokio::time::timeout(Duration::from_secs(5), rx.recv()).await?.unwrap();
